...
export class OrderService {
    ...
    async updateOrderStatus(id, status, paymentMethod) {
        if (status === 'DELIVERED') {
            const order = await this.repos.orderRepo.getById(id);
            if (order) {
                const deliveredItems = order.items.map((it) => ({ ...it, status: 'DELIVERED' }));
    ...
    async updateItemStatus(orderId, itemId, itemStatus) {
        ...
        const newOrderStatus = allReady ? 'READY' : order.status === 'PENDING' ? 'PREPARING' : order.status;
